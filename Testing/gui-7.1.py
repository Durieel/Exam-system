...
def utskrift_eksamen_pd(tree3, fra_aar_utskriftside3, fra_m_utskriftside3, fra_d_utskriftside3, til_aar_utskriftside3,
    ...
    if til_dato == '':
        ...
        # Lager variabel for MySQL-kode
        a_utskrift = (
            'SELECT Eksamen.Emnekode, Emnenavn, Eksamen.Romnr, Antallplasser, Eksamen.dato '
            'FROM Eksamen, Rom, Emne '
            'WHERE Eksamen.Dato = %s '
        ...
        # Utfører handlingen i databasen
        marker.execute(a_utskrift, b_utskrift)
        mydb.commit()
...
def add_stud():
    ...
    marker_sjekk.execute('SELECT Studentnr '
                         'FROM Student')

    liste = []
    for row in marker_sjekk:
        liste = list(row)
        print('hei')
    ...
    m_stud = ('INSERT INTO Student(StudentNr, Fornavn, Etternavn, Epost, Telefon) '
              'SELECT * FROM (SELECT "100000", %s, %s, %s, %s) as tmp '
              'WHERE NOT EXISTS ( '
              'SELECT Studentnr '
              'FROM Student); ')
    d_stud = (fornavn, etternavn, epost, telefon)
