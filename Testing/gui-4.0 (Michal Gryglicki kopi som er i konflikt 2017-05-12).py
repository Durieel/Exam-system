...
def utskrifter():
    ...
    p1.grid(row=2, column=0, padx=10, pady=5, rowspan=2, columnspan=2, sticky=W)
    ...
    p1 = ttk.Panedwindow(f2, orient=VERTICAL, width=220, height=45)
    p1.grid(row=2, column=0, padx=10, pady=5, rowspan=2, sticky=W)
    l1 = ttk.Labelframe(p1, text='Info', width=340, height=55)
    p1.add(l1)
    label = Label(l1, justify='left', text="Skriv inn emnekode og klikk 'Utfør'")
    label.grid(row=0, column=0)

    ttk.Label(f2, text='Studentnr:').grid(row=2, column=2, padx=90, pady=5, sticky=E)
    ttk.Entry(f2, textvariable=studentnr_2, width=9).grid(row=2, column=2, padx=5, pady=5, sticky=E)

    ttk.Button(f2, text='Utfør', command=lambda: utskrift_eks_student(tree2)).grid(row=3, column=1, columnspan=2, padx=5,
                                                                                   pady=5)
