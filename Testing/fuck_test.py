...
# GUI for vitnemål
def vitnemal(listbox):
    # Oppretter en toplevel-frame
    vit = Toplevel()
    vit.title('Eksamensadministrasjonsapplikasjon')

    ttk.Label(vit, text="Utskrift av vitnemål for en student").grid(column=0, columnspan=8, row=0, pady=15, padx=15,
                                                            sticky=(W, E))
    ttk.Separator(vit).grid(row=1, column=0, columnspan=8, pady=5, sticky=(W, E))

    # Lables og Entries
    ttk.Label(vit, text='Studentnr:').grid(row=2, column=0, padx=5, pady=7, sticky=W)
    ttk.Entry(vit, textvariable=studentnr_1, width=6).grid(row=2, column=1, padx=5, pady=7, sticky=W)



    # Oppretter Panedwindow for å legge inn veiledende tekst
    p = ttk.Panedwindow(vit, orient=VERTICAL)
    ...
    # Oppretter listbox for visning av vitnemål fra databasen
    listbox = Listbox(vit, height=20)
    listbox.grid(row=5, column=0, columnspan=5, sticky=(N, W, E, S))

    s = ttk.Scrollbar(vit, orient=VERTICAL, command=listbox.yview)
    s.grid(row=5, column=4, sticky=(N, S, E))
    listbox['yscrollcommand'] = s.set
    ttk.Sizegrip().grid(row=5, column=1, sticky=(S, E))
    vit.grid_columnconfigure(1, weight=1)
    vit.grid_rowconfigure(1, weight=1)

    # Oppretter button som kjører mysql-queries mot databasen
    ttk.Button(vit, text='Utfør', command=listbox(listbox)).grid(row=4, column=0, columnspan=2, pady=15, padx=5,
                                                                 sticky=W)

    # Lager button som gjør at brukeren kan gå tilbake til hovedmeny
    ttk.Button(vit, text='Lukk vinduet', command=vit.destroy).grid(row=9, column=4, padx=5, pady=15, sticky=E)
