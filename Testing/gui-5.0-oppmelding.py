...
def utskrift_eksamen_pd(fra_dag, fra_maaned, fra_aar, til_dag, til_maaned, til_aar, tree3):
    ...
    if til_dato == '':
        ...
        m_utskrift = ('SELECT Eksamen.Emnekode, Emnenavn, Eksamen.Romnr, Antallplasser, Eksamen.dato, COUNT(Oppmeldte.Studentnr) '
                      ...
                      'AND Oppmeldte.Studentnr >= Rom.Antallplasser '
...
def utskrift_stud_emne(tree5):
    ...
    m_stud_emne = ('SELECT Student.Studentnr, Fornavn, Etternavn, Emne.Emnekode, Emnenavn '
                ...
                'AND %s IN (SELECT Emnekode FROM Eksamensresultat) '
                'AND Studentnr IN (SELECT Studentnr FROM Eksamensresultat) '
...
def utskrifter():
    ...
    ttk.Label(utskrift, text='Oversikt over utskrifter', font=('Calibri', '16')).grid(row=0, column=0, columnspan=2, pady=10)
    ...
    ttk.Label(f1, text="Alle eksamensresultater i et emne", font=('Calibri', '14')).grid(column=0, columnspan=3, row=0, pady=15, padx=15)
    ...
    ttk.Label(f2, text="Eksamensresultater for en student", font=('Calibri', '14')).grid(column=0, columnspan=3, row=0, pady=15, padx=15)
    ...
    ttk.Entry(f2, textvariable=studentnr_2, width=9).grid(row=2, column=2, padx=10, pady=5, sticky=E)
    ...
    # Side 3 i notebook er Utskrift av eksamner i en periode/dag
    ttk.Label(f3, text="Utskrift av eksamner i en periode/dag", font=('Calibri', '14')).grid(column=0, columnspan=4, row=0, pady=15, padx=15)
    ...
    # Side 4 i notebook er karakterstatistikk for en eksamen
    ttk.Label(f4, text='Karakterstatistikk for en eksamen', font=('Calibri', '14')).grid(column=0, columnspan=4, row=0, pady=15, padx=15)
    ...
    ttk.Entry(f4, textvariable=emne1, width=14).grid(row=3, column=2, columnspan=2, padx=10, pady=5, sticky=E)

    ttk.Label(f4, text='Dato:').grid(row=4, column=2, columnspan=2, padx=140, pady=5, sticky=E)
    # fra dato

    dato01 = StringVar()
    dato01_entry = ttk.Entry(f4, textvariable=dato01, width=5).grid(column=2, row=4, columnspan=2, padx=80, pady=5,
                                                                    sticky=(N, E))
    dato02 = StringVar()
    dato02_entry = ttk.Entry(f4, textvariable=dato02, width=3).grid(column=2, row=4, columnspan=2, padx=45, pady=5,
                                                                    sticky=(N, E))
    dato03 = StringVar()
    dato03_entry = ttk.Entry(f4, textvariable=dato03, width=3).grid(column=2, row=4, columnspan=2, padx=10, pady=5,
                                                                    sticky=(N, E))
    ...
    ttk.Button(f4, text='Utfør', command=lambda: utskrift_karstat(dato01, dato02, dato03, tree4)).grid(
    ...
    #Labels, entries og buttons
    ttk.Label(f5, text="En oversikt som viser studentene i et emne", font=('Calibri', '14')).grid(column=0, columnspan=3, row=0, pady=15,
                                                                          padx=15)
    ...
    ttk.Entry(f5, textvariable=emne_2, width=9).grid(row=2, column=2, padx=10, pady=5, sticky=E)

    ttk.Button(f5, text='Utfør', command=lambda: utskrift_stud_emne(tree5)).grid(row=3, column=2, padx=7,
                                                                                 pady=15, sticky=E)
    ...
    ttk.Label(f6, text="Utskrift av vitnemål for en student", font=('Calibri', '14')).grid(column=0, columnspan=4, row=0, pady=15, padx=15)
...
# MySQL-kode som endrer karakter til student
def upd_karakter():
    marker = mydb.cursor()
...
# GUI for administrering av karakterer
def adm_kar_gui():
    adm_kar = Toplevel()
    adm_kar.title('Administrasjon av Karakterer')

    ttk.Label(adm_kar, text='Administrering av Karakter', font=('Calibri', '16')).grid(column=0, columnspan=1, row=0, pady=15, padx=15,
                                                               sticky='WE')
    ...
    #Side 1 i notebook endring av karakter på en student
    ...
    #Side 2 i notebook er registrer karakterer for avholdt eksamen
...
# MySQL-kode som endrer en oppsatt eksamen
...
# MySQL-kode som skriver ut alle oppsatte eksamner
...
# MySQL-kode som lager en tabell 'oppmeldte' og legger inn antall oppmeldte kandidater til eksamen
def oppmelding_eksamen(emne_kode_opp, dato_opp, studentnr_opp):
    marker = mydb.cursor()

    emnekode = emne_kode_opp.get()
    dato = dato_opp.get()
    studentnr = studentnr_opp.get()
...
# MySQL-kode som skriver ut diverse utskrifter fra tabellen 'oppmeldte'
def utskrift_opp(tree_opp, emne_kode_opp, dato_opp, studentnr_opp):
    ...
    emne_opp = emne_kode_opp.get()
    dato_oppmelding = dato_opp.get()
    studentnr_oppmelding = studentnr_opp.get()

    if emne_opp == '' and dato_oppmelding == '' and studentnr_oppmelding == '':
        ...
        if emne_opp != '' and dato_oppmelding != '' and studentnr_oppmelding == '':
            ...
            d_stud_emne = (emne_opp, dato_oppmelding)
            ...
            if emne_opp == '' and dato_oppmelding == '' and studentnr_opp != '':
                m_stud_emne = (
                'SELECT * '
                'FROM Oppmeldte '
                'WHERE Studentnr =%s '
                ...
                if emne_opp != '' and dato_oppmelding != '' and studentnr_oppmelding != '':
                    ...
                    d_stud_emne = (emne_opp, dato_oppmelding, studentnr_oppmelding)
...
def adm_eksamen_gui():
    ...
    ttk.Label(adm_eks, text='Administrering av Eksamen', font=('Calibri', '16')).grid(column=0, columnspan=2, row=0, pady=15, padx=15,
                                                              sticky='WE')

    # Oppretter en Notebook-widget
    n = ttk.Notebook(adm_eks)
    n.grid(row=1, column=0, rowspan=2, sticky='NESW')
    f1 = ttk.Frame(n)  # første side
    f2 = ttk.Frame(n)  # andre side
    f3 = ttk.Frame(n)
    ...
    ttk.Button(f2, text='Endre eksamen',
               command=lambda: eksamen_endre(til_dato1, til_dato2, til_dato3, fra_dato1, fra_dato2, fra_dato3, fra_rom,
                                             til_rom)).grid(row=7, column=2, padx=5, pady=5, sticky=W)


    # Side 3 i notebook
    ttk.Label(f3, text='Emnekode:').grid(row=1, column=0, rowspan=3, padx=5, pady=5, sticky=N)
    emne_kode_opp = StringVar()
    ttk.Entry(f3, textvariable=emne_kode_opp, width=15).grid(row=1, column=1, rowspan=3, padx=5, pady=5, sticky=N)

    ttk.Label(f3, text='Dato:').grid(row=1, column=0, rowspan=3, padx=5, pady=35, sticky=(N, W))
    dato_opp = StringVar()
    ttk.Entry(f3, textvariable=dato_opp, width=15).grid(row=1, column=1, rowspan=3, padx=5, pady=35, sticky=N)

    ttk.Label(f3, text='Studentnr:').grid(row=1, column=0, rowspan=3, padx=5, pady=65, sticky=N)
    studentnr_opp = StringVar()
    ttk.Entry(f3, textvariable=studentnr_opp, width=15).grid(row=1, column=1, rowspan=3, padx=5, pady=65, sticky=N)

    ttk.Button(f3, text='Meld opp til eksamen', command=lambda: oppmelding_eksamen(emne_kode_opp, dato_opp, studentnr_opp))\
        .grid(row=1, column=1, rowspan=3, padx=5, pady=95, sticky=N)

    ttk.Label(f3, justify='left', text='Oversikt over alle \n oppmeldte elever').grid(row=1, column=3, padx=25,
                                                                                       pady=10, sticky=(N, W))
    ttk.Button(f3, text='Klikk her!', command=lambda: utskrift_opp(tree_opp, emne_kode_opp, dato_opp, studentnr_opp)).grid(row=1, column=3, padx=40,
                                                                                   pady=50, sticky=(N, W))
    ...
    label = Label(l1, justify='left', text="For å melde opp student til eksamen, \n"
                                           ...
                                           "* For å sjekke en spesifikk eksamen, fyll inn dato og emnekode og trykk 'klikk her'\n"
                                           "* For å sjekke en spesifikk student, fyll inn studentnr og trykk 'klikk her'\n"
                                           "* For å sjekke en spesifikk student opp mot en spesifikk eksamen, fyll inn alle felter og trykk 'klikk her'\n"
                                           "* For å sjekke alle studenter som er oppmeldt til eksamen, la alle feltene stå blankt og trykk 'klikk her'")
    label.grid(row=0, column=0)
    ...
    ttk.Button(adm_eks, text='Lukk vinduet', command=adm_eks.destroy).grid(row=3, column=0, padx=5, pady=5, sticky=E)


# MySQL-kode for utskrift av alle rom
...
# MySQL-kode som legger inn rom i databasen
def add_rom():
    marker = mydb.cursor()
...
def adm_rom_gui():
    ...
    ttk.Label(adm_rom, text="Administrering av Rom", font=('Calibri', '16')).grid(column=0, columnspan=2, row=0, pady=15, padx=15, sticky='WE')
...
# MySQL-kode for utskrift av alle emner
...
def adm_emne_gui():
    ...
    ttk.Label(adm_emne, text="Administrering av Emne", font=('Calibri', '16')).grid(column=0, columnspan=3, row=0, pady=15, padx=15,
                                                            sticky=(W, E))
...
# MySQL-kode som legger inn student i databasen
def add_stud():
    # Oppretter en cursor
    marker = mydb.cursor()
...
# MySQL-kode som sletter student fra databasen
def delete_stud():
    # Oppretter en cursor
    marker = mydb.cursor()
...
def adm_stud_gui():
    ...
    ttk.Label(adm_stud, text="Administrering av Studenter", font=('Calibri', '16')).grid(column=0, row=0, pady=15, padx=15, sticky=(W, E))
...
ttk.Label(mainframe, text="Administrering av eksamen ved HSN", font=('Calibri', '24')).grid(column=0, row=0, columnspan=3, pady=50)
ttk.Separator(mainframe, orient=HORIZONTAL).grid(column=0, row=1, columnspan=3, sticky=(W, E))

ttk.Label(mainframe, text="Administrer Studenter", font=1).grid(column=0, row=2, sticky=(E, W))
ttk.Button(mainframe, text="Studenter", width=30, command=adm_stud_gui).grid(column=1, row=2, padx=20, sticky=(N, S))

ttk.Label(mainframe, text="Administrer Emner", font=1).grid(column=0, row=3, sticky=(E, W))
ttk.Button(mainframe, text="Emner", width=30, command=adm_emne_gui).grid(column=1, row=3, padx=20, sticky=(N, S))

ttk.Label(mainframe, text="Administrer Rom", font=1).grid(column=0, row=4, sticky=(E, W))
ttk.Button(mainframe, text="Rom", width=30, command=adm_rom_gui).grid(column=1, row=4, padx=20, sticky=(N, S))

ttk.Label(mainframe, text="Administrer Eksamener", font=1).grid(column=0, row=5, sticky=(E, W))
ttk.Button(mainframe, text="Eksamener", width=30, command=adm_eksamen_gui).grid(column=1, row=5, padx=20, sticky=(N, S))

ttk.Label(mainframe, text="Administrer Karakterer", font=1).grid(column=0, row=6, sticky=(E, W))
ttk.Button(mainframe, text="Karakterer", width=30, command=adm_kar_gui).grid(column=1, row=6, padx=20, sticky=(N, S))

ttk.Separator(mainframe, orient=HORIZONTAL).grid(column=0, row=7, columnspan=3, pady=50, sticky=(W, E))

ttk.Label(mainframe, text="Utskrifter av oversikter", font=1).grid(column=0, row=8, sticky=(E, W))
ttk.Button(mainframe, text="Oversikter", width=30, command=utskrifter).grid(column=1, row=8, padx=20, sticky=(N, S))

ttk.Separator(mainframe, orient=HORIZONTAL).grid(column=0, row=9, columnspan=3, pady=20, sticky=(W, E))
ttk.Button(mainframe, text="Avslutt", command=root.destroy, width=25).grid(column=2, row=10, padx=20, sticky=(N, S, E))
...
tekst6.insert('1.0', 'På utskrift av oversikter kan du få diverse utskrifter av karakter')
