...
def utskrifter():
    ...
    ttk.Button(f3, text='Utfør',
               command=lambda: utskrift_eksamen_pd(tree3, fra_aar_utskriftside3, fra_m_utskriftside3,
                                                   fra_d_utskriftside3,
...
def add_eksres(stud, kar, dato_aar_karakteradd, dato_m_karakteradd, dato_d_karakteradd):
    stud.bind()
    kar.bind()
...
def adm_kar_gui():
    ...
    button = ttk.Button(f2, text='Legg til eksamensresultat',
                        command=lambda: add_eksres(stud, kar, dato_aar_eksamenadd, dato_m_eksamenadd,
                                                   dato_d_eksamenadd))
...
# Opprette "Oppmeldte" table
def opprette_oppmeldte():
    # Oppretter cursor
    marker = mydb.cursor()
...
def oppmelding_eksamen(emnekode_eksamenoppmelding, dato_aar_eksamenoppmelding, dato_m_eksamenoppmelding,
                       dato_d_eksamenoppmelding,
...
#
def avmelding_eksamen(emnekode_eksamenoppmelding1, dato_aar_eksamenoppmelding1,
                                                  dato_m_eksamenoppmelding1, dato_d_eksamenoppmelding1,
                                                  studentnr_eksamenoppmelding1):
    # Oppretter cursor
    marker = mydb.cursor()

    # Henter strengene og lagrer de som variabler
    emnekode = emnekode_eksamenoppmelding1.get()
    dato_aar = dato_aar_eksamenoppmelding1.get()
    dato_m = dato_m_eksamenoppmelding1.get()
    dato_d = dato_d_eksamenoppmelding1.get()
    studentnr = studentnr_eksamenoppmelding1.get()

    dato = dato_aar + dato_m + dato_d

    # Setter opp database strukturen for tabellen
    m_del = ('DELETE FROM Oppmeldte '
              'WHERE Emnekode = %s AND Dato = %s AND Studentnr = %s')
    d_del = (emnekode, dato, studentnr)

    # Legger strengene inn i databasen
    marker.execute(m_del, d_del)
    mydb.commit()
...
def utskrift_opp(tree_opp, emnekode_eksamenoppmelding, dato_aar_eksamenoppmelding, dato_m_eksamenoppmelding,
                 dato_d_eksamenoppmelding,
                 studentnr_eksamenoppmelding):
    # Binder Treeviewet
    tree_opp.bind()
...
def adm_eksamen_gui():
    ...
    n.add(f4, text='Avmelding fra eksamen')
    ...
    ttk.Button(f3, text='Meld opp til eksamen',
               command=lambda: oppmelding_eksamen(emnekode_eksamenoppmelding, dato_aar_eksamenoppmelding,
                                                  dato_m_eksamenoppmelding, dato_d_eksamenoppmelding,
                                                  studentnr_eksamenoppmelding)) \
    ...
    ttk.Button(f3, text='Klikk her!',
               command=lambda: utskrift_opp(tree_opp, emnekode_eksamenoppmelding, dato_aar_eksamenoppmelding,
                                            dato_m_eksamenoppmelding, dato_d_eksamenoppmelding,
                                            studentnr_eksamenoppmelding)) \
    ...
    # Side 4 i notebook er avmelding av eksamen
    # Oppretter Panedwindow for Notebook side 1 og legger inn veiledende tekst
    p1 = ttk.Panedwindow(f4, orient=VERTICAL)
    p1.grid(row=0, column=2, rowspan=2, padx=20, pady=10)
    l1 = ttk.Labelframe(p1, text='Info', width=150, height=100)
    p1.add(l1)
    label = Label(l1, justify='left', text="Fyll ut alle feltene og klikk \n"
                                           "'Meld av fra eksamen'")
    label.grid(row=0, column=0)

    ttk.Label(f4, text='Emnekode:').grid(row=1, column=0, rowspan=3, padx=5, pady=5, sticky=N)
    emnekode_eksamenoppmelding1 = StringVar()
    ttk.Entry(f4, textvariable=emnekode_eksamenoppmelding1, width=15) \
        .grid(row=1, column=1, rowspan=3, padx=5, pady=5, sticky=(N, W))

    ttk.Label(f4, text='Dato:').grid(row=1, column=0, rowspan=3, padx=5, pady=35, sticky=(N, W))
    dato_aar_eksamenoppmelding1 = StringVar()
    ttk.Entry(f4, textvariable=dato_aar_eksamenoppmelding1, width=5).grid(column=1, row=1, rowspan=3, padx=5, pady=35,
                                                                         sticky=(N, W))
    dato_m_eksamenoppmelding1 = StringVar()
    ttk.Entry(f4, textvariable=dato_m_eksamenoppmelding1, width=3).grid(column=1, row=1, rowspan=3, padx=47, pady=35,
                                                                       sticky=(N, W))
    dato_d_eksamenoppmelding1 = StringVar()
    ttk.Entry(f4, textvariable=dato_d_eksamenoppmelding1, width=3).grid(column=1, row=1, rowspan=3, padx=76, pady=35,
                                                                       sticky=(N, W))

    ttk.Label(f4, text='Studentnr:').grid(row=1, column=0, rowspan=3, padx=5, pady=65, sticky=N)
    studentnr_eksamenoppmelding1 = StringVar()
    ttk.Entry(f4, textvariable=studentnr_eksamenoppmelding1, width=15) \
        .grid(row=1, column=1, rowspan=3, padx=5, pady=65, sticky=(N, W))

    ttk.Button(f4, text='Meld av fra eksamen',
               command=lambda: avmelding_eksamen(emnekode_eksamenoppmelding1, dato_aar_eksamenoppmelding1,
                                                  dato_m_eksamenoppmelding1, dato_d_eksamenoppmelding1,
                                                  studentnr_eksamenoppmelding1)) \
        .grid(row=1, column=1, rowspan=3, padx=5, pady=95, sticky=(N, W))
...
def add_stud():
    ...
    # Setter opp database strukturen for tabellen
    # Oppretter en "Test-Student" hvis det ikke finnes noen studenter fra før for å få startet Studentnr på 100000
    m_stud = ('INSERT INTO Student(StudentNr, Fornavn, Etternavn, Epost, Telefon) '
              'SELECT * FROM (SELECT "100000", "Test", "Student", "test@student.no", "12345678") as tmp '
              'WHERE NOT EXISTS ( '
              'SELECT Studentnr '
              'FROM Student ' 
              'WHERE Studentnr="100000");')

    a_stud = ('INSERT INTO '
              'Student(StudentNr, Fornavn, Etternavn, Epost, Telefon) '
              'SELECT MAX(Studentnr) + 1, %s, %s, %s, %s '
              'FROM Student '
              'WHERE EXISTS ( '
              'SELECT Studentnr '
              'FROM Student '
              'WHERE Studentnr="100000");')
    g_stud = (fornavn, etternavn, epost, telefon)

    # Legger strengene inn i databasen
    marker.execute(m_stud)
    marker.execute(a_stud, g_stud)
    mydb.commit()
...
# MySQL-kode for oversikt av alle studenter
def oversikt_student(tree_student):
    # Henter Treeviewet
    tree_student.bind()

    for i in tree_student.get_children():
        tree_student.delete(str(i))

    # Oppretter en cursor
    marker = mydb.cursor()

    # Setter opp database strukturen for tabellen
    marker.execute('SELECT * '
                   'FROM Student')

    # Henter informasjon fra databsen inn i Treeviewet
    for row in marker:
        tree_student.insert('', 'end', values=(row[0], row[1], row[2], row[3], row[4]))
...
def adm_stud_gui():
    ...
    n.add(f3, text='Oversikt over alle studenter')
    ...
    p1.add(l1)
    label = Label(l1, justify='left', text="Fyll inn Fornavn, Etternavn, E-post \n"
                                           "og Telefonnr til studenten, \n"
                                           "og klikk 'Legg til student'!\n"
                                           "NB! Studentnr trengs kun ved oppdatering\n"
                                           "av student! Studentnr vil ble generert \n"
                                           "automatisk")
    label.grid(row=0, column=0)
    ...
    # Side 3 i administer student er oversikt over alle studenter
    # Lager Treeview-widget
    tree_student = ttk.Treeview(f3)

    tree_student["columns"] = ("1", "2", "3", "4", "5")
    tree_student.column("#0", anchor="w", width=0)
    tree_student.column("#1", width=100)
    tree_student.column("#2", width=100)
    tree_student.column("#3", width=100)
    tree_student.column("#4", width=150)
    tree_student.column("#5", width=100)
    tree_student.heading("#1", text="Studentnr")
    tree_student.heading("#2", text="Fornavn")
    tree_student.heading("#3", text="Etternavn")
    tree_student.heading("#4", text="E-post")
    tree_student.heading("#5", text="Telefon")

    # Lager scrollbar til viewet
    ysb = ttk.Scrollbar(f3, orient=VERTICAL, command=tree_student.yview)

    tree_student['yscroll'] = ysb.set

    # add tree and scrollbars to frame
    tree_student.grid(row=1, column=0, columnspan=4, sticky=NSEW)
    ysb.grid(row=1, column=4, sticky=(N, S, E))

    ttk.Label(f3, text="Trykk 'Klikk her!' for oversikt over alle studenter").grid(row=0, column=2)

    # Lager button som skriver ut oversikt over alle emner
    ttk.Button(f3, text='Klikk her!', command=lambda: oversikt_student(tree_student)).grid(row=0, column=3, padx=25,
                                                                                           pady=30,
                                                                                           sticky=(N, E))
...
ttk.Button(mainframe, text="Oversikter", width=30, command=lambda: [utskrifter(), opprette_oppmeldte()]) \
...
tekst1 = Text(mainframe, width=48, height=4, wrap="word", bg='lightgrey')
...
# Gir child elementene i mainframe litt "space"
